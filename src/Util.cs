...
namespace JudesEquipment
{
    public static class Util
    {
        ...
        public static readonly int[] TorsoUpperArms =
            Torso.Concat(ArmUpperLeft).Concat(ArmUpperRight).ToArray();
        public static readonly int[] TorsoUpperLowerArms =
            Torso.Concat(ArmUpperLeft).Concat(ArmLowerLeft)
            .Concat(ArmUpperRight).Concat(ArmLowerRight).ToArray();

        public static readonly int[] CompleteLegs =
            LegUpperLeft.Concat(LegLowerLeft).Concat(FootLeft)
            .Concat(LegUpperRight).Concat(LegLowerRight).Concat(FootRight).ToArray();

        public static AssetBundle LoadBundle(string bundleName)
        {
            return AssetBundle.LoadFromStream(
                Main.assembly.GetManifestResourceStream(
                    Main.assembly.GetName().Name + ".Resources." + bundleName));
        }
        ...
        public static bool IsModAvailable(string modName, string minVersion, bool printAllMods = false)
        {
            return Chainloader.PluginInfos.Values.ToList()
                .Find(_modName => _modName.Metadata.Name == modName)?
                .Metadata.Version >= new System.Version(minVersion);
        }
        ...
        public static List<HitData.DamageModPair> ParseDmgModPairs(string def)
        {
            try
            {
                ...
                foreach (string pair in splitPairs)
                {
                    ...
                    HitData.DamageType dmgType = (HitData.DamageType)Enum.Parse(
                        typeof(HitData.DamageType), splitPair[0].ToString().Trim());
                    HitData.DamageModifier dmgMod = (HitData.DamageModifier)Enum.Parse(
                        typeof(HitData.DamageModifier), splitPair[1].ToString().Trim());
